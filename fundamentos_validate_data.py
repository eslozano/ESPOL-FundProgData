...
TIPO_NO_DECIMAL = 'dato_no_decimal'
	...
	try:
		...
		for row in data_sheet.iter_rows(min_row=header_rows+1, max_col=total_columns, max_row=data_sheet.max_row):
			...
			for cell in row:
				...
				fillCell(cell,colores[errorsMessages[0]]) #Restore cell color
		...
		print("Naranja(vacio), Azul(opcion_no_valida), Amarillo(fuera_de_rango) o Verde(necesita_mejoamiento).")
		wb.save(filename)
	except IOError as e:
...
#Escriba el nombre del archivo de datos
filename = "PAR7_2017II.xlsx"
...
columnas = ["nombre","matricula","genero","paralelo","cod_carrera","veces_tomadas",
			...
			"calif_final_practica","AP_sin_mejoramiento",
...
colors = {
	...
	NO_REDONDEADO: '7cff94', #verde
	TIPO_NO_DECIMAL: 'ff7ff4' #rosado
}



errorsMessages= [VACIO,OPCION_NO_VALIDA,FUERA_DE_RANGO,NO_REDONDEADO,TIPO_NO_DECIMAL]
...
validaciones = {	
	...
	"examen":{
		"1er_exam_": { "tema1": 20, "tema2": 30, "tema3": 45, "tema4": 10 },
